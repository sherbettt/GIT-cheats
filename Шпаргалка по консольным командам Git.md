...
git tag -a v1.0.2 -m "Версия 1.0.2" abc1234
```

Push тегов на удаленный репозиторий
```bash
# Один конкретный тег
git push origin v1.0.0

# Отправить ВСЕ локальные теги на origin
...
```bash
# Вариант 1: Получить ВСЕ теги с origin
git fetch --tags

# Вариант 2: Получить все изменения, включая теги
git fetch --all --tags
...
# Проверить, какие теги есть на удаленном репозитории
git ls-remote --tags origin

# Сравнить локальные и удаленные теги
git tag | sort > local_tags.txt
git ls-remote --tags origin | awk '{print $2}' | sed 's/refs\/tags\///' | sort > remote_tags.txt
...
спасибо https://github.com/cyberspacedk
